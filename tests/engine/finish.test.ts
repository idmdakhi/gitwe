...
describe("Engine.finish", () => {
  ...
  it("tags a release and back-merges into the auto-updating child", async () => {
    ...
    config.versioning = {
      ...
      tag: ["release", "hotfix"],
      bumpRules: { major: [], minor: ["release"], patch: ["hotfix"] },
      path: ".gitwe/VERSION.yaml",
      autoCommit: true,
      commitMessage: "chore: bump version to {{version}}",
      initialVersion: "0.1.0",
    };
    const engine = await repo.engine(config, true);

    // ایجاد فایل‌های نسخه‌بندی
    const gitDir = join(repo.path, ".gitwe");
    if (!existsSync(gitDir)) mkdirSync(gitDir, { recursive: true });
    writeFileSync(join(gitDir, "VERSION.yaml"), 'version: "1.1.0"\n', "utf8");
    writeFileSync(join(repo.path, "package.json"), JSON.stringify({ version: "1.1.0" }), "utf8");
    repo.git("add", ".");
    ...
    expect(result.tag).toBe("v1.2.0"); // چون از 1.1.0 به 1.2.0 می‌رود (minor)
