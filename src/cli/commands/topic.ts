...
//     return;
//   }
//   const type = typeName === undefined ? undefined : engine.workflow.requireBranchType(typeName);
...
//       const engine = await createEngine(globals());
//       const branches = await engine.listBranchTypes(type, pattern);
...
//       const engine = await createEngine(globals());
//       const topic = await engine.resolveTarget(type, name);
//       const result = await engine.deleteBranchType(topic, options);
...
//       const [from, to] =
//         newName === undefined
//           ? [(await engine.currentBranchType()).branch, oldName]
//           : [oldName, newName];
...
// ): void {
//   for (const type of config.branchTypes) registerTopicType(program, type, globals);
// }
// -------------------------------------------------------
// -------------------------------------------------------
// -------------------------------------------------------
